...
export default function ListColumn({ list, dragHandleProps, search, filter, onOpenCard }: ListColumnProps) {
  ...
  return (
    <div className="flex w-[272px] shrink-0 flex-col rounded-xl bg-surface shadow-sm">
      ...
      >
        <GripVertical size={14} className="text-ink-faint opacity-70" />
        {editing ? (
          <div className="flex items-center gap-1">
            <input
              ...
              onBlur={commitRename}
              autoFocus
              className="w-32 rounded-md px-1 py-0.5 text-sm font-semibold text-ink outline-none ring-1 ring-brand"
            />
            <button
              type="button"
          ...
          <h3
            ...
            className="flex-1 cursor-text truncate text-sm font-semibold text-ink"
          >
            {list.name}
          </h3>
        )}
        <span className="font-mono text-[10.5px] text-ink-faint">{allCards.length}</span>
        <ListMenu list={list} />
      </div>

      {list.assignee && (
        <p className="px-3 pb-1.5 text-[11px] text-ink-muted">by {list.assignee}</p>
      )}
      ...
      <Droppable droppableId={list.id} type="CARD">
        {(droppableProvided, snapshot) => (
          <div
            ...
            className={`scroll-slim flex max-h-[calc(100vh-180px)] min-h-2 flex-1 flex-col gap-1.5 overflow-y-auto rounded-b-xl px-2 pb-2 transition-colors ${
