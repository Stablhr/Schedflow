...
export default function CaptureBox({ autoFocus = false, variant = 'sidebar' }: { autoFocus?: boolean; variant?: 'sidebar' | 'dash' }) {
  ...
  return (
    <div
      className={`flex items-center gap-1.5 rounded-xl bg-surface shadow-sm ring-1 ring-border transition focus-within:ring-2 focus-within:ring-brand ${variant === 'sidebar' ? 'p-2' : 'p-2.5'}`}
    >
      <button
        type="button"
        onClick={submit}
