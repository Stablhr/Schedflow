  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex h-full gap-4 overflow-hidden p-4">
        ...
        <div className="w-[180px] shrink-0 rounded-xl glass-subtle p-2 shadow-sm sm:w-[230px] sm:p-3">
          ...
          <div className="flex flex-col gap-1.5">
            {Array.from({ length: 3 }, (_, i) => (
              <div key={i} className="rounded-lg glass-subtle p-2 shadow-sm">
        ...
        <div className="flex min-w-0 flex-1 gap-2">
          {Array.from({ length: 7 }, (_, day) => (
            <div key={day} className="flex min-w-0 flex-1 flex-col">
              {/* Day header */}
              <div className={`mb-2 rounded-lg px-2 py-1.5 text-center ${day === 0 ? 'bg-brand' : 'bg-surface shadow-sm'}`}>
                <Skeleton className={`mx-auto h-2.5 w-5 ${day === 0 ? 'opacity-50' : ''}`} />
                <Skeleton className={`mx-auto mt-1 h-4 w-4 ${day === 0 ? 'opacity-50' : ''}`} />
              </div>

              {/* Cards area */}
              <div className="min-h-[120px] flex-1 rounded-xl p-1.5 glass-subtle">
                <div className="flex flex-col gap-1.5">
